 ...
 */
package controller.managementFunction;
...
public class ProcessRequest extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<RequisForm> list = formDAO.getAllFormByIdUserAcceptForm(users.getIdUser());
        request.setAttribute("listFormProcess", list);
        if (list.isEmpty()) {
            request.setAttribute("EmptyListProcess", "Empty list process");
        }

        request.getRequestDispatcher("root/display/management/processRequest.jsp").forward(request, response);
    }
